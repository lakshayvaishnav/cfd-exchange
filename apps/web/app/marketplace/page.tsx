...
import { createChart, CrosshairMode, IChartApi, CandlestickData, UTCTimestamp, CandlestickSeries } from "lightweight-charts";
...
const Marketplace = () => {
  ...
  const { data, isLoading, isError } = useGetCandles(selectedInterval, 0, 0, "BTC_USDC");
  ...
  return (
    <div className="w-full h-screen bg-white flex flex-col">
          ...
          <div className="hidden md:flex md:items-center md:justify-between">
            <div className="flex items-center gap-6 lg:gap-8">
              <nav className="flex items-center gap-6 lg:gap-8">
                ...
                <Link href="/docs" className="text-gray-600 hover:text-black transition-colors font-instrument-sans">
                  Docs
                </Link>
              ...
              <div className="flex items-center gap-4 lg:gap-6 pl-6 lg:pl-8 border-l border-gray-300">
                <div className="flex items-center gap-3">
                  ...
                  <IntervalSelector selectedInterval={selectedInterval} onIntervalChange={setSelectedInterval} />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 lg:gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-lg font-semibold text-black font-ibm-plex-mono">{balanceLoading ? "Loading..." : `$${totalBalance.toLocaleString()}`}</span>
              </div>
              <button onClick={() => setIsDepositModalOpen(true)} className="bg-black text-white text-sm px-6 py-2 rounded-4xl font-medium transition-colors hover:bg-gray-800 cursor-pointer">
                Deposit
              </button>
            </div>
          </div>
          ...
          <div className={`md:hidden ${mobileMenuOpen ? "block" : "hidden"}`}>
            <div className="space-y-4 pt-4 border-t border-gray-200">
              <nav className="flex flex-col gap-3 pb-4 border-b border-gray-200">
                <Link href="/" className="text-gray-600 hover:text-black transition-colors font-instrument-sans" onClick={() => setMobileMenuOpen(false)}>
                  Home
                </Link>
                <Link href="/docs" className="text-gray-600 hover:text-black transition-colors font-instrument-sans" onClick={() => setMobileMenuOpen(false)}>
                  Docs
                </Link>
                <Link href="/marketplace" className="text-black font-semibold font-instrument-sans">
                  Marketplace
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-semibold text-white font-ibm-plex-mono">{balanceLoading ? "Loading..." : `$${totalBalance.toLocaleString()}`}</span>
                </div>

                <div className="flex flex-col gap-2">
                  <span className="text-sm text-gray-600">Timeframe:</span>
                  <IntervalSelector selectedInterval={selectedInterval} onIntervalChange={setSelectedInterval} className="w-full" />
                </div>

                <div className="pt-2">
                  <button onClick={() => setIsDepositModalOpen(true)} className="w-full text-white text-sm px-4 py-3 rounded-4xl font-medium transition-colors hover:bg-gray-800">
                    Deposit
                  </button>
                </div>
              </div>
      ...
      <div className="flex-1 overflow-hidden pb-20 md:pb-0" style={{ height: "calc(100vh - 120px)" }}>
