...
import { useGetOrders, useCloseOrder } from "../hooks/useOrder";
...
const OrdersSection: React.FC = () => {
    ...
    currentOrders.forEach((currentOrder: Order) => {
      ...
      if (prevOrder && prevOrder.status === "open" && currentOrder.status === "closed") {
        ...
        if (currentOrder.closeReason === "Liquidation" || currentOrder.closeReason === "margin") {
          toast.error(`Order Liquidated: ${currentOrder.symbol.toUpperCase()} ${currentOrder.orderType.toUpperCase()} | PnL: ${pnlText}`, { duration: 6000 });
        } else if (currentOrder.closeReason === "TakeProfit") {
          toast.success(`Take Profit Hit: ${currentOrder.symbol.toUpperCase()} ${currentOrder.orderType.toUpperCase()} | PnL: ${pnlText}`, { duration: 5000 });
        } else if (currentOrder.closeReason === "StopLoss") {
          toast.error(`Stop Loss Hit: ${currentOrder.symbol.toUpperCase()} ${currentOrder.orderType.toUpperCase()} | PnL: ${pnlText}`, { duration: 5000 });
        }
      }
    });
  ...
  const currentPrices = useMemo(() => {
    ...
    if (orderBook && orderBook.symbol) {
      ...
      const bid = orderBook.bids?.length > 0 ? parseFloat(orderBook.bids[0]?.[0] || "0") : null;
      const ask = orderBook.asks?.length > 0 ? parseFloat(orderBook.asks[0]?.[0] || "0") : null;
    ...
    if (ordersData?.orders) {
      ordersData.orders.forEach((order: Order) => {
        ...
        if (!priceMap.has(symbol) && !priceMap.has(symbolWithUsdc)) {
          const tradePrice = tradeMap.get(symbol) || tradeMap.get(symbolWithUsdc);
          if (tradePrice) {
            priceMap.set(symbol, {
              bid: tradePrice,
              ask: tradePrice,
  ...
  const filteredOrders = activeTab === "open" ? orders.filter((order: Order) => order.status === "open") : orders;
    ...
    return (
      <div className="bg-white border-t border-gray-200 p-4">
        <div className="text-center">
          ...
          <button onClick={() => window.location.reload()} className="mt-2 px-4 py-2 bg-black text-white rounded-4xl text-sm">
            Retry
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0">
        <div className="flex space-x-4">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-4xl transition-colors ${activeTab === "open" ? "bg-black-100 text-black-700" : "text-gray-600 hover:text-gray-900"}`}
          >
            Open Orders ({orders.filter((o: Order) => o.status === "open").length})
          </button>
          <button
            onClick={() => setActiveTab("all")}
            className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === "all" ? "bg-black-100 text-black-700" : "text-gray-600 hover:text-gray-900"}`}
          >
            ...
            <table className="w-full min-w-max">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Open Price</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Close Price</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Take Profit</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Stop Loss</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOrders.map((order: Order) => {
                  ...
                  return (
                    <tr key={`${order.id}-${order.createdAt}`} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{order.symbol.toUpperCase()}</td>
                      <td className="px-4 py-3 text-sm">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${order.orderType === "long" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                      ...
                      <td className="px-4 py-3 text-sm font-mono text-gray-900">${currentPrice && typeof currentPrice === "number" ? currentPrice.toFixed(4) : "---"}</td>
                      ...
                      <td className="px-4 py-3 text-sm font-mono text-gray-900">{order.leverage ? `${order.leverage}x` : "1x"}</td>
                      <td className="px-4 py-3 text-sm font-mono text-gray-900">
                        {order.takeProfit && order.takeProfit > 0 ? `$${typeof order.takeProfit === "number" ? order.takeProfit.toFixed(4) : parseFloat(order.takeProfit).toFixed(4)}` : "---"}
                      </td>
                      <td className="px-4 py-3 text-sm font-mono text-gray-900">
                        {order.stopLoss && order.stopLoss > 0 ? `$${typeof order.stopLoss === "number" ? order.stopLoss.toFixed(4) : parseFloat(order.stopLoss).toFixed(4)}` : "---"}
                      </td>
                      <td className="px-4 py-3 text-sm">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${order.status === "open" ? "bg-yellow-100 text-yellow-800" : "bg-gray-100 text-gray-800"}`}>
                              ...
                              order.closeReason === "TakeProfit"
                                ...
                                : order.closeReason === "StopLoss"
                                  ...
                                  : order.closeReason === "Manual"
                                    ...
                                    : order.closeReason === "Liquidation" || order.closeReason === "margin"
                                      ? "bg-orange-100 text-orange-800"
                                      : "bg-gray-100 text-gray-800"
                            }`}
                          >
                            {order.closeReason === "margin" ? "Liquidation" : order.closeReason}
                          </span>
                        ) : (
                          "---"
                        )}
