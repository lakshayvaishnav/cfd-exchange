  ...
  return (
    <div className="p-3 border-b border-gray-200 hover:bg-gray-50 transition-colors">
      <div className="flex justify-between items-start mb-2">
        <span className="font-medium text-gray-900 text-sm">{symbol.replace("_", "/")}</span>
      </div>

      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="flex flex-col">
          <span className="text-gray-500 text-xs">Bid</span>
          <span className="font-mono text-green-600">${bid != null ? bid.toLocaleString() : lastTradePrice.toLocaleString()}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-gray-500 text-xs">Ask</span>
          <span className="font-mono text-red-600">${ask != null ? ask.toLocaleString() : lastTradePrice.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
...
const LeftSideBar: React.FC = () => {
  ...
  const symbolData = React.useMemo(() => {
    ...
    return Array.from(dataMap.values()).sort((a, b) => a.data.s.localeCompare(b.data.s));
  }, [messages]);
  ...
  return (
    <div className="w-full bg-white border-r border-gray-200 h-full flex flex-col">
      <div className="p-4 border-b border-gray-200 bg-gray-50">
        <h2 className="text-lg font-semibold text-gray-900 font-ibm-plex-mono">Live Ticker</h2>
        <p className="text-sm text-gray-600 mt-1">{symbolData.length} symbols • Live Bid/Ask</p>
      </div>
