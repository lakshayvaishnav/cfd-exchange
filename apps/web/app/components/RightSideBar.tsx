    ...
    <>
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-300 p-4 z-40">
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => setIsMobileTradeOpen(true)} className="bg-black text-white py-3 px-4 rounded-4xl font-medium transition-colors hover:bg-gray-800">
            Long
          </button>
          <button onClick={() => setIsMobileTradeOpen(true)} className="bg-white text-gray-900 border-2 border-gray-900 py-3 px-4 rounded-4xl font-medium transition-colors hover:bg-gray-100">
            Short
          </button>
        </div>
      </div>
        ...
        <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-50">
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-lg max-h-[80vh] overflow-hidden">
            <div className="p-4 border-b border-gray-300 bg-white flex justify-between items-center">
              <h2 className="text-lg font-semibold text-black font-ibm-plex-mono">Trade {selectedSymbol.toUpperCase()}</h2>
              <button onClick={() => setIsMobileTradeOpen(false)} className="p-2 hover:bg-gray-100 rounded-4xl">
            ...
            <div className="overflow-y-auto max-h-[calc(80vh-80px)]">
              <div className="p-4 border-b border-gray-300">
                ...
                <div className="text-xs text-gray-600 mt-2 text-center">Long orders pay ask price • Short orders receive bid price</div>
              </div>

              <div className="p-4 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-900 mb-2">Volume/Quantity</label>
                  <input
                    type="text"
                    value={volume}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-900 mb-2">Leverage: {leverage}x</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-900 mb-2">Take Profit (Optional)</label>
                  <div className="relative">
                    <input
                      ...
                      placeholder="Not set"
                      className={`w-full px-3 py-2 pr-8 border rounded-4xl font-mono ${takeProfit && !validateTakeProfit(takeProfit) ? "border-red-500 bg-red-50" : "border-gray-300"}`}
                    />
                    {takeProfit && (
                      <button type="button" onClick={() => setTakeProfit("")} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-900 mb-2">Stop Loss (Optional)</label>
                  <div className="relative">
                    <input
                      type="text"
                      inputMode="decimal"
                      ...
                      placeholder="Not set"
                      className={`w-full px-3 py-2 pr-8 border rounded-4xl font-mono ${stopLoss && !validateStopLoss(stopLoss) ? "border-red-500 bg-red-50" : "border-gray-300"}`}
                    />
                    {stopLoss && (
                      <button type="button" onClick={() => setStopLoss("")} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                        </svg>
                      </button>
                  ...
                  <div className="mt-4 p-3 bg-gray-100 border border-gray-900 rounded-4xl">
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono text-gray-900">${getExecutionPrice(orderType)?.toFixed(4)}</span>
                      </div>
                      <hr className="my-2 border-gray-300" />
                      <div className="flex justify-between font-medium">
                        <span className="text-gray-700">Notional:</span>
                        <span className="font-mono text-gray-900">${(parseFloat(volume) * (getExecutionPrice(orderType) || 0)).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between font-medium">
                        <span className="text-gray-700">Required Margin:</span>
                        <span className="font-mono text-gray-900">${((parseFloat(volume) * (getExecutionPrice(orderType) || 0)) / leverage).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                )}
      ...
      <div className="hidden md:flex w-full bg-white border-l border-gray-300 h-full flex-col">
        ...
        <div className="p-4 border-b border-gray-300 flex-shrink-0">
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              <div className="text-sm text-gray-600 mb-1">Bid</div>
              <div className="text-lg font-mono font-semibold text-green-600">${currentSymbolData?.bid ? currentSymbolData.bid.toFixed(4) : lastTradePrice ? lastTradePrice.toFixed(4) : "---"}</div>
            </div>
            <div className="text-center">
              <div className="text-sm text-gray-600 mb-1">Ask</div>
              <div className="text-lg font-mono font-semibold text-red-600">${currentSymbolData?.ask ? currentSymbolData.ask.toFixed(4) : lastTradePrice ? lastTradePrice.toFixed(4) : "---"}</div>
            </div>
          </div>
          <div className="text-xs text-gray-600 mt-2 text-center">Long orders pay ask price • Short orders receive bid price</div>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Not set"
                  className={`w-full px-3 py-2 pr-8 border rounded-lg font-mono ${takeProfit && !validateTakeProfit(takeProfit) ? "border-red-500 bg-red-50" : "border-gray-300"}`}
                />
                {takeProfit && (
                  <button type="button" onClick={() => setTakeProfit("")} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Not set"
                  className={`w-full px-3 py-2 pr-8 border rounded-lg font-mono ${stopLoss && !validateStopLoss(stopLoss) ? "border-red-500 bg-red-50" : "border-gray-300"}`}
                />
                {stopLoss && (
                  <button type="button" onClick={() => setStopLoss("")} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
